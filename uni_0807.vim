...
if 1
  packadd EasyBuffer
  nnoremap <silent> <C-k> :<C-u>EasyBufferToggle<CR>
  nnoremap <silent> <expr> <C-k> ':<C-u>EasyBuffer' . ( <SID>WindowRatio() >=  0 ? 'Vertical' : 'Horizontal' ) . '<CR>'
  nnoremap <silent> <C-k> :<C-u>EasyBufferBotRight<CR>:OptimalWindowHeight<CR>
endif
...
if has('win32')
  ...
  let s:FontHVRatio	= 0.5   " 縦横比
endif

function! ResizeFont(r)
  if a:r != 0
    let s:CurFontHeight += a:r
    let s:CurFontWidth  += a:r * s:FontHVRatio
...
" mapping
"nnoremap <Home>		:<C-u>call ResizeFont(+0.5)<CR>:XFont<CR>
"nnoremap <End>		:<C-u>call ResizeFont(-0.5)<CR>:XFont<CR>
"nnoremap <C-Home>	:<C-u>call ResizeFont(0)<CR>:XFont<CR>
"nnoremap <C-End>	:<C-u>call ResizeFont(0)<CR>:XFont<CR>
"nnoremap <S-End>	:<C-u>call ResizeFont(-99999)<CR>:XFont<CR>

" HHKB
nnoremap <A-q>	:<C-u>call ResizeFont(+0.5)<CR>:XFont<CR>
nnoremap <A-w>	:<C-u>call ResizeFont(-0.5)<CR>:XFont<CR>
nnoremap <A-e>	:<C-u>call ResizeFont(0)<CR>:XFont<CR>
nnoremap <A-r>	:<C-u>call ResizeFont(-99999)<CR>:XFont<CR>

" initialize
"exe 'set guifont=' . s:FontName . ':h' . s:FontHeight . ':w' s:FontWidth
if !exists('g:font_init_done')
" FullHD NotePC {{{
  let s:FontHeight	= 11.5
  let s:FontWidth	= 6
" FullHD NotePC }}}
...
if 1  " HHKB
  nnoremap <silent><expr> <F11> ':<C-u>se transparency=' . max([&transparency - 1,   1]) . '<Bar> Transparency <CR>'   | " transparencyは、0以下を設定すると255になってしまう。
 "nnoremap <silent><expr> <A--> ':<C-u>se transparency=' . max([&transparency - 1,   1]) . '<Bar> Transparency <CR>'   | " transparencyは、0以下を設定すると255になってしまう。
  nnoremap <silent><expr> <F12> ':<C-u>se transparency=' .    ( &transparency + 1      ) . '<Bar> Transparency <CR>'
 "nnoremap <silent><expr> <A-=> ':<C-u>se transparency=' .    ( &transparency + 1      ) . '<Bar> Transparency <CR>'

  nnoremap <silent>       <S-F11> :<C-u>exe 'se transparency=' . (&transparency == g:my_transparency ?  50 : g:my_transparency) <Bar> Transparency <CR>
  nnoremap <silent>       <S-F12> :<C-u>exe 'se transparency=' . (&transparency == g:my_transparency ? 255 : g:my_transparency) <Bar> Transparency <CR>
endif
...
if has('kaoriya')
  ...
  nnoremap <silent> <F6> :<C-u>Think<CR>

  nnoremap <silent> <F7> :<C-u>XGA<CR>

  com! OriginalScreen ScreenMode 0
  nnoremap <silent> <F8> :<C-u>OriginalScreen<CR>
...
inoremap <expr> <C-u> TriggerComplete("\<C-x>\<C-u>", '', 'x') . Selsel()
