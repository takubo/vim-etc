...
let g:SearchCWordEchoFuncName = v:true
...
function! s:SearchCWord(add, aword, keep_pos, strict_case, proc_top_underscore)
  ...
  if a:add
    ...
    let @/ = (a:strict_case == s:CaseStrict ? '\C' : a:strict_case == s:CaseIgnore ? '\c' : '') . search_str
  endif
...
" CONST
"
unlockvar s:SearchNew s:SearchAdd
unlockvar s:MatchWord s:MatchPart
unlockvar s:CursorKeep s:CursorMove
unlockvar s:CaseIgnore s:CaseStrict s:CaseIgnore
...
lockvar s:SearchNew s:SearchAdd
lockvar s:MatchWord s:MatchPart
lockvar s:CursorKeep s:CursorMove
lockvar s:CaseIgnore s:CaseStrict s:CaseIgnore
