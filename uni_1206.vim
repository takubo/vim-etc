...
nnoremap <silent> <C-g><C-g> :<C-u>exe 'cd ' GetPrjRoot() <Bar> grep '\<<C-R><C-W>\>' -- ':!.svn/' ':!tags'<CR>
...
" 5 範囲     cur_file, cur_dir, cur_dir&childs, prj, input
" 3 拡張子   all, cur(c,h), input
" 4 対象     input, cword, register, search
...
function! DiffSpecial()
  if &diff | diffupdate | echo 'diffupdate' | return | endif
...
" VertSplit すべきとき正数が返る。HorizontalSplit すべきとき、負数が返る。
...
com! -nargs=1 -complete=custom,VimrcContents V silent 1 <Bar> call search(<q-args> . ' {{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{', 'cW')
...
hi SpecialKey	guifg=#343434	guibg=grey20	gui=none
hi SpecialKey	guifg=#454545	guibg=grey20	gui=none
hi LineNr	guifg=#5c5a4f	guibg=grey20	gui=none	ctermfg=239	ctermbg=232
hi LineNr	guifg=#5c574f	guibg=grey20	gui=none	ctermfg=239	ctermbg=232
hi LineNr	guifg=#6c6a5f	guibg=grey20	gui=none	ctermfg=239	ctermbg=232
endif
