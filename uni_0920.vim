...
function! s:SetDefaultStatusline(statusline_contents)
 ...
 "let s:stl .= "%##%m%r%{(!&autoread&&!&l:autoread)?'[AR]':''}%h%w"
  ...
  let s:stl .= "%#SLFileName# [ %{winnr()} ] %#VertSplit# ( %n ) "
  let s:stl .= "%#VertSplit#%m%r%{(!&autoread&&!&l:autoread)?'[AR]':''}%h%w"
...
nnoremap <buffer> <expr> <Leader>e (&ro ? '' : ':<C-u>w<CR>') . ':' . ( match(getcwd(), '/colors$') == -1 ? 'so %' : 'colorscheme ' . expand('%:r') ) . '<CR>'
nnoremap <buffer> <expr>        ge (&ro ? '' : ':<C-u>w<CR>') . ':' . ( match(getcwd(), '/colors$') == -1 ? 'so %' : 'colorscheme ' . expand('%:r') ) . '<CR>'
+++++ plugin/PrjTree.vim ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
scriptencoding utf-8
" vim: set ts=8 sts=2 sw=2 tw=0 :
...
" General colors
hi Normal	guifg=#f6f3f0	guibg=#282828	gui=none	ctermfg=254	ctermbg=235
"hi Normal	guifg=#448280	guibg=#d0c589	gui=none	ctermfg=242	ctermbg=237	" 金箔背景	平家納経
hi Normal	guifg=#282828	guibg=#d0c589	gui=none	ctermfg=242	ctermbg=237	" 金箔背景
hi Comment 	guifg=#f6f3f0			ctermfg=244	| " gui=italic
hi SpecialKey	guifg=#848484	guibg=NONE	gui=none
hi CursorColumn	guifg=NONE	guibg=NONE	gui=NONE			ctermbg=236
hi LineNr	guifg=#5c5a4f	guibg=#1a1a1a	gui=none
hi LineNr	guifg=#5c5a4f	guibg=#d0c589	gui=none
